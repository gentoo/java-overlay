...
bootstrap_uri() {
	local baseuri="https://dev.gentoo.org/~arthurzam/distfiles/dev-java/${MY_PN}/${MY_PN}-bootstrap"
	local suff="tar.xz"
...
SRC_URI="
	https://github.com/${MY_PN}/jdk/archive/jdk-${MY_PV}.tar.gz
...
pkg_pretend() {
	...
	if [[ ${MERGE_TYPE} != binary ]]; then
		has ccache ${FEATURES} && die "FEATURES=ccache doesn't work with ${MY_PN}, bug #677876"
	fi
}
...
pkg_setup() {
	...
	# /opt/${MY_PN}-${SLOT} or if there was a mechanism to install a VM env
...
src_configure() {
	...
	# Zero VM specific (https://openjdk.org/projects/zero/)
	# zero jvm requires buildin with libffi
	zero+=(
		--with-jvm-variants=zero
		--with-libffi="$($(tc-getPKG_CONFIG) --variable=prefix libffi)"
		--with-libffi-include="$($(tc-getPKG_CONFIG) --variable=includedir libffi)"
		--with-libffi-lib="$($(tc-getPKG_CONFIG) --variable=libdir libffi)"
	)
...
src_install() {
	...
	java-vm_install-env "${FILESDIR}"/${MY_PN}.env.sh
	java-vm_revdep-mask
