...
DESCRIPTION="Java bindings for the Cairo graphics libraries"
...
RDEPEND=">=dev-java/glib-java-0.2.1
...
src_install() {
	...
	cd ${S}/src/java
	find . -name '*.java' | xargs zip ${D}/usr/share/${PN}-${SLOT}/src/cairo-java-${PV}.src.zip
	...
	echo "CLASSPATH=/usr/share/${PN}-${SLOT}/lib/cairo${SLOT}.jar" \
		>> ${D}/usr/share/${PN}-${SLOT}/package.env
}
