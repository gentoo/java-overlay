...
#	epatch ${FILESDIR}/${P}-picocontainer.patch
...
src_compile() {
	...
	antflags="${antflags} -Dmx4j.home=/usr/share/mx4j-core/lib"
	antflags="${antflags} -lib $(java-config -p picocontainer-1)"
	# Need to up maximum memory to avoid OutOfMemoryErrors
	ANT_OPTS="${ANT_OPTS} -Xmx512m" eant ${antflags}
