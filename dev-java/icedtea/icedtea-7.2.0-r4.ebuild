...
IUSE="+X +alsa cjk +cups debug doc examples -jamvm javascript +jbootstrap
	+nsplugin +nss pax_kernel pulseaudio +source -shark systemtap test +webstart -zero"
REQUIRED_USE="
	jamvm? ( jbootstrap )"
	#cacao? ( jbootstrap )
...
java_prepare() {
	...
	pushd ../${ICEDTEA_PKG} > /dev/null || die
		...
		epatch "${FILESDIR}"/${P}_shark_llvm_flags.patch #IT639
	...
	if use shark && [[ "$(llvm-config --version)" == 3.0 ]]; then
		...
		local llvm_patches=(
			...
			# Uncomment next line to enable experimental MCJIT support
...
src_configure() {
	...
	if use cacao || use jamvm; then
		if [[ ${bootstrap} ]]; then
			...
			ewarn "Can't build CACAO or JamVM without bootstrap."
...
src_install() {
	...
	dodoc ../${ICEDTEA_PKG}/{README,NEWS,AUTHORS}
	dosym /usr/share/doc/${PF} /usr/share/doc/${PN}${SLOT}

	# JamVM hack (remove when no longer needed)
