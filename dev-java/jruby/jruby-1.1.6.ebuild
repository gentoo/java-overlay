...
EANT_GENTOO_CLASSPATH="asm-3 bytelist constantine jcodings jline joda-time joni jna jna-posix jvyamlb"
...
java_prepare() {
	# Patches from the RDT project.
	epatch "${FILESDIR}/rdt-${PV}/"*.patch
	cp "${FILESDIR}/rdt-${PV}/ReaderLexerSource.java" src/org/jruby/lexer/yacc || die

	# Make jruby-1.1.6 work with joni-1.1.3.
	epatch "${FILESDIR}/joni-${PV}.patch"
