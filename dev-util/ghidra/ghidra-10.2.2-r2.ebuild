...
src_prepare() {
	default
	local repos_gradle="${EGRADLE_USER_HOME}/init.d/repos.gradle"
	install -D "${FILESDIR}"/repos.gradle "${repos_gradle}" || die
	sed -i "s|S_DIR|${S}|g" "${repos_gradle}" || die
...
src_compile() {
	egradle prepDev -x check -x test
	egradle buildGhidra -x check -x test --parallel
