...
NB_FILE="${PN}-${MY_PV}-m7-src-200702191730-ide_sources-19_Feb_2007_1731.tar.bz2"
MOBILITY_FILE="${PN}-mobility-${MY_PV}-${MILESTONE}.tar.bz2"
PROFILER_FILE="profiler-60dev-linux.bin"
SRC_URI="http://us1.mirror.netbeans.org/download/${MY_PV}/${MILESTONE}/200702191730/${NB_FILE}
	mobility? ( http://dev.gentoo.org/~fordfrog/distfiles/${MOBILITY_FILE} )
	profiler? ( http://us1.mirror.netbeans.org/download/${MY_PV}/${MILESTONE}/200702191730/${PROFILER_FILE} )"
LICENSE="CDDL"
KEYWORDS="~amd64 ~x86 ~x86-fbsd"
IUSE="debug doc mobility profiler"
...
src_unpack () {
	...
	if use profiler ; then
		einfo "Unpacking ${PROFILER_FILE}"
		mkdir ${WORKDIR}/profiler
		cd ${WORKDIR}/profiler

		# Walk-around: copy installer to working-directory
		cp ${DISTDIR}/${PROFILER_FILE} ${WORKDIR}/profiler/ || die "Cannot copy ${PROFILER_FILE}"
		chmod u+x ${PROFILER_FILE} || die "Cannot chmod ${PROFILER_FILE}"
		...
		./${PROFILER_FILE} -silent || die "Running ${PROFILER_FILE} failed"

		# Walk-around: remove installer again
		rm ${PROFILER_FILE} || die "Cannot remove ${PROFILER_FILE}"

		# remove unnecessary uninstall informations
		rm -R profiler1/_uninst || die "Cannot remove _uninst from profiler"
	fi
...
src_install() {
	...
	# Update info about profiler module
	if use profiler ; then
		echo profiler1 >> ${D}/${CLUSTER_FILE} || die "Cannot update ${CLUSTER_FILE}"
		echo NB_PROF >> ${D}/${PRODUCTID_FILE} || die "Cannot update ${PRODUCTID_FILE}"
	fi
...
function place_unpack_symlinks() {
	...
	#ide/bluej-suite/branding/modules/org-netbeans-core.jar
	#ide/bluej-suite/branding/modules/org-netbeans-core-windows.jar
	#ide/bluej-suite/branding/modules/org-netbeans-modules-utilities.jar

	# java
	#java/external/javac-api.jar
	#java/external/javac-impl.jar
	#java/external/lucene-core-2.0.0.jar
	...
	java-pkg_jar-from commons-logging commons-logging-api.jar commons-logging-1.1.jar
	...
	#xml/wsdlui/external/org-netbeans-api-visual.jar
}
